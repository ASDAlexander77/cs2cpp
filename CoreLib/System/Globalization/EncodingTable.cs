    ...
    internal static class EncodingTable
    {
        ...
        unsafe private static int internalGetCodePageFromName(String name)
        {
            ...
            while ((right - left) > 3)
            {
                ...
                result = name.CompareTo(encodingDataPtr[index].webName);
            ...
            for (; left <= right; left++)
            {
                if (name.CompareTo(encodingDataPtr[left].webName) == 0)
                {
                    return (encodingDataPtr[left].codePage);
                }
            }
            // The encoding name is not valid.
            throw new ArgumentException("EncodingNotSupported");
        }
        ...
        **  internalGetCodePageFromName will throw ArgumentException if name is not a valid encoding name.
        ...
        internal unsafe static byte* nativeCreateOpenFileMapping(
