                ...
                while ((ch = GetNextChar()) != 0)
                {
                    ...
                    if (Char.IsSurrogate(ch))
                    {
                        if (Char.IsHighSurrogate(ch))
                        {
                            // High Surrogate
                            if (bHighSurrogate)
                                throw new ArgumentException("InvalidCharSequenceNoIndex");
                            bHighSurrogate = true;
                        }
                        else
                        {
        ...
        internal void ThrowLastBytesRecursive(byte[] bytesUnknown)
        {
            ...
            for (i = 0; i < bytesUnknown.Length && i < 20; i++)
            {
                if (strBytes.Length > 0)
                    strBytes.Append(" ");
                strBytes.Append(String.Format("\\x{0:X2}", bytesUnknown[i]));
            }
            ...
            throw new ArgumentException("RecursiveFallbackBytes");
        }
    }
}
