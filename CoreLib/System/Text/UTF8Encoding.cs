...
namespace System.Text
{
    ...
    public class UTF8Encoding : Encoding
    {
        ...
        public override unsafe int GetByteCount(String chars)
        {
            ...
            fixed (char* pChars = &chars.ToCharArray()[0])
        ...
        internal override unsafe int GetByteCount(char* chars, int count, EncoderNLS baseEncoder)
        {
            ...
            for (; ; )
            {
                ...
                if (pSrc >= pEnd)
                {
                    ...
                    else
                    {
                        ...
                        if (fallbackBuffer != null && fallbackBuffer.bFallingBack)
                        {
                            ...
                            if (InRange(ch, Char.LOW_SURROGATE_START, Char.LOW_SURROGATE_END))
                            {
                                ch = 0xfffd;
                                byteCount++;
                                goto EncodeChar;
                ...
                if (ch > 0)
                {
                    ...
                    if (InRange(cha, Char.LOW_SURROGATE_START, Char.LOW_SURROGATE_END))
                    {
            ...
            ProcessChar:
                ...
                if (InRange(ch, Char.HIGH_SURROGATE_START, Char.HIGH_SURROGATE_END))
                {
            ...
            // either good char or partial surrogate
                ...
                if (InRange(ch, Char.HIGH_SURROGATE_START, Char.LOW_SURROGATE_END))
                {
                    ...
                    if (ch > 0x7FF)
                    {
                        ...
                        if (InRange(ch, Char.HIGH_SURROGATE_START, Char.LOW_SURROGATE_END))
                        {
                            ...
                            if (
                                ...
                                ch > Char.HIGH_SURROGATE_END ||
                                // !IsLowSurrogate(chd) // high not followed by low -> bad
                                !InRange(chd, Char.LOW_SURROGATE_START, Char.LOW_SURROGATE_END))
                            {
                ...
                if (pSrc >= pEnd)
                {
                    ...
                    else
                    {
                        ...
                        if (fallbackBuffer != null && fallbackBuffer.bFallingBack)
                        {
                            ...
                            if (InRange(ch, Char.LOW_SURROGATE_START, Char.LOW_SURROGATE_END))
                            {
                                ch = ch + (cha << 10) + (0x10000 - Char.LOW_SURROGATE_START - (Char.HIGH_SURROGATE_START << 10));
                                goto EncodeChar;
                            }
                            else if (ch > 0)
                            {
                ...
                LongCode:
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (!InRange(chd, Char.LOW_SURROGATE_START, Char.LOW_SURROGATE_END))
                            {
                ...
                if ((ch & (SupplimentarySeq | 0x1F0000)) > SupplimentarySeq)
                {
                    ...
                    if (pTarget < pAllocatedBufferEnd)
                    {
                        *pTarget = (char)(((ch >> 10) & 0x7FF) +
                            unchecked((short)((Char.HIGH_SURROGATE_START - (0x10000 >> 10)))));
                        pTarget++;

                        ch = (ch & 0x3FF) +
                            unchecked((int)(Char.LOW_SURROGATE_START));
                    }
                }
            ...
            EncodeChar:
                ...
                if (pTarget >= pAllocatedBufferEnd)
                {
                    ...
                    if (ch > 0x7f)
                    {
                        if (ch > 0x7ff)
                        {
                            if (ch >= Char.LOW_SURROGATE_START &&
                                ch <= Char.LOW_SURROGATE_END)
                            {
                                pSrc--;     // It was 4 bytes
                    ...
                    if ((ch & 0x20) != 0)
                    {
                        ...
                        if ((ch & 0x10) != 0)
                        {
                            ...
                            *pTarget = (char)(((ch >> 10) & 0x7FF) +
                            ...
                            ch = (ch & 0x3FF) +
                                unchecked((short)(Char.LOW_SURROGATE_START));
        ...
        public override int GetMaxByteCount(int charCount)
        {
            ...
            if (byteCount > 0x7fffffff)
                throw new ArgumentOutOfRangeException("charCount", "GetByteCountOverflow");
        ...
        public override int GetMaxCharCount(int byteCount)
        {
            ...
            if (charCount > 0x7fffffff)
                throw new ArgumentOutOfRangeException("byteCount", "GetCharCountOverflow");
