...
template <typename D, typename S>
inline typename std::enable_if<is_class_type<S>::value && is_interface_type<D>::value, D>::type constrained_ref(S& s)
{
...
template <typename D, typename S>
inline typename std::enable_if<is_value_type<S>::value && is_interface_type<D>::value, D>::type constrained_ref(S& s)
{
	return interface_cast<D>(s);
}
...
public:
	...
	string* operator() (const char16_t* str, size_t length)
	{
		...
		string::wstrcpy(&_new_string->FIRST_CHAR_FIELD, (char16_t*)str, length);
