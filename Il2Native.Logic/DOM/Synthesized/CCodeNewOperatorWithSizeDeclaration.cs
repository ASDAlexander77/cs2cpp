...
namespace Il2Native.Logic.DOM.Synthesized
{
    ...
    {
        public CCodeNewOperatorWithSizeDeclaration(INamedTypeSymbol type, bool withFinalization = false)
            : base(new NewOperatorMethod(type))
        {
            ...
            MethodBodyOpt = new MethodBody(Method)
            {
                Statements =
                {
                    new ReturnStatement { ExpressionOpt = new Call { Method = methodSymbol, Arguments = { parameter, new Literal { Value = ConstantValue.Create(false) } } } }
                }
            };
        }
